...
const ShopDetail: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const [fetchedShops, fetchedBills] = await Promise.all([
        api.getShops(user.email),
        api.getBills(user.email)
      ]);
      const currentShop = (fetchedShops || []).find(s => String(s.id) === String(id));
      if (currentShop) {
        setShop(currentShop);
        // Parse items if they come as string from backend, though typically handled in API layer
        const parsedBills = (fetchedBills || []).map(b => ({
          ...b,
          items: typeof b.items === 'string' ? JSON.parse(b.items) : b.items
        }));
        setBills(parsedBills.filter(b => String(b.shopId) === String(id)));
      } else {
        navigate('/shops');
      }
    } catch (error) {
      console.error("Shop Detail Load Error:", error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSaveBill = async (e: React.FormEvent) => {
    ...
    try {
      await api.saveBill(user.email, billData); 
      await fetchData(); 
  ...
  const handleDeleteBill = async (billId: string) => {
    ...
    try { await api.deleteBill(user.email, billId); await fetchData(); } catch (error) { alert("Delete failed."); }
  };

  const handleDeleteShop = async () => {
    if (!user || !id || !window.confirm(t('confirmDelete'))) return;
    try { await api.deleteShop(user.email, id); navigate('/shops'); } catch (error) { alert("Delete failed."); }
  };
        ...
        <div className="space-y-3">
          {pagedBills.length === 0 ? (
            ...
            pagedBills.map(bill => {
              ...
              const updated = (bill.updatedAt && bill.updatedAt !== bill.createdAt) ? getSessionInfo(bill.updatedAt) : null;
